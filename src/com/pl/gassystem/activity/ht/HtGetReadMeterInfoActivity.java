                ...
                OkHttpUtils.post()
                        ...
                        .execute(new StringCallback() {
                            ...
                            public void onResponse(String response, int id) {
                                LogUtil.i("获取需要补抄1\n" + response);
                                LogUtil.i("获取需要补抄2" + xml2JSON(response));

                                Gson gson = new Gson();
                                HtGetReadMeterInfo info = gson.fromJson(xml2JSON(response), HtGetReadMeterInfo.class);
                                mList.clear();
                                if(info.getArrayOfModTimereadmeterinfo().getModTimereadmeterinfo()!=null){
                                    mList.addAll(info.getArrayOfModTimereadmeterinfo().getModTimereadmeterinfo());
