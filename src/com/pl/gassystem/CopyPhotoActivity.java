...
public class CopyPhotoActivity extends Activity {
	...
	private TextView tvTitlebar_name, tvPhotoBtInfo, tvCopyPhotoBackMsg,
			tvLoadingPhotoComNum;
	private CopyBiz copyBiz;
	private String meterNo;
	private String meterTypeNo, baseType, YHTM, XBDS, MQBBH, HUNANME, OTEL,
			ADDR;
	private ImageButton btnCopyPhotoScan, btnCopyPhotoRead, btnOnlybackQuit;
	private ImageView imgCopyPhotoImage;
	private ImageLoader loader;
	private TextView tvCopyPhotoOcrRead, tvCopyPhotoOcrState,
			tvCopyPhotoDevState, tvCopyPhotoDevPower;
	private ProgressBar pgbCopyPhoto;
	private EditText etCopyPhotoImgPosition, etCopyPhotoSetPoint;
	private Button btnCopyPhotoImgUp, btnCopyPhotoImgDown,
			btnCopyPhotoSetPoint;
	...
	private void query() {
		...
		if ((commNumber.length() != 10)) {
			Toast.makeText(getApplicationContext(), "通讯编号错误，非10位!",
	...
	private void sendMessage(String message) {
		...
		if (message.length() > 0) {
            ...
            if (htLogDao == null) {
                htLogDao = new HtLogDao(CopyPhotoActivity.this);
            }
            htLogDao.putSXLog(currentTimeMillis() + "", "0", message);
		}
	}
		...
		btnCopyPhotoSetPoint.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (tvCopyPhotoBackMsg.getText().toString().equals("表具已唤醒正在抄表")
					...
					if (poi.length() != 6) {
						Toast.makeText(getApplicationContext(), "输入错误，必须6位！",
								Toast.LENGTH_SHORT).show();
					} else {
	...
	private void getCopyDataPhoto(String postData, String meterTypeNo) {
		...
		client.post(url, params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
				...
				try {
					...
					if (copyDataPhoto != null) {
						...
						in.close();
						Message msg = Message.obtain(mHandler,
								MESSAGE_GETCOPYDATAPHOTO_SUCCESS);
						mHandler.sendMessage(msg);
					} else {
						Message msg = Message.obtain(mHandler,
								MESSAGE_GETCOPYDATAPHOTO_ERROR);
						mHandler.sendMessage(msg);
					}
			...
			public void onFailure(int arg0, Header[] arg1, byte[] arg2,
					Throwable arg3) {
				Message msg = Message.obtain(mHandler,
						MESSAGE_GETCOPYDATAPHOTO_FAILURT);
				mHandler.sendMessage(msg);
			}
		});
	}
	...
	private void getCopy(String getMsg) {
		try {
			if (gettimes == 0) { // 获取第一包数据
				if (getMsg.substring(0, 2).equals("68")
						&& getMsg.substring(getMsg.length() - 2).equals("16")) { // 第一包数据。
					firstString = getMsg;
					gettimes++;
				}
			} else {
	...
	private void fillCopy(String getMsg) {
		if (getMsg.length() == 116) {
			...
			page = page - 1;
			ImgString = ImgString.substring(0, 117 * page) + getMsg + ";"
					+ ImgString.substring(117 * page);
			isFill = false;
			pgbCopyPhoto.incrementProgressBy(1);
			// 再次检测
			if (!checkFill()) {
			...
			int page = Integer.parseInt(getMsg.substring(6, 8), 16); // 获取当前包号
			page = page - 1;
			ImgString = ImgString.substring(0, 485 * page) + getMsg + ";"
					+ ImgString.substring(485 * page);
			isFill = false;
			pgbCopyPhoto.incrementProgressBy(1);
			// 再次检测
			if (!checkFill()) {
	...
	private void showCopyDataPhoto() {
		...
		tvCopyPhotoOcrState.setText(MeterType
				.GetCopyPhotoOcrState(copyDataPhoto.getOcrState()));
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_STATE_CHANGE:
				...
				case BluetoothChatService.STATE_CONNECTED:
					tvPhotoBtInfo
							.setText(getString(R.string.title_connected_to)
			...
			case MESSAGE_READ:
                ...
                htLogDao.putSXLog(currentTimeMillis() + "", "1", readMessage);
