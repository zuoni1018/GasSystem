...
public class GroupInfoDao {
    ...
    public ArrayList<GroupInfo> getGroupInfos(String BookNo) {
        ...
        Cursor cursor = db.rawQuery("select * from GroupInfo where bookNo = ? order by groupNo desc", new String[]{BookNo});
        if (cursor != null) {
            groupInfos = new ArrayList<>();
            while (cursor.moveToNext()) {
                GroupInfo gpInfo = new GroupInfo();
                gpInfo.setGroupNo(cursor.getString(cursor.getColumnIndex("groupNo")));
                gpInfo.setEstateNo(cursor.getString(cursor.getColumnIndex("estateNo")));
                gpInfo.setGroupName(cursor.getString(cursor.getColumnIndex("groupName")));
                gpInfo.setRemark(cursor.getString(cursor.getColumnIndex("Remark")));
                gpInfo.setMeterTypeNo(cursor.getString(cursor.getColumnIndex("meterTypeNo")));
